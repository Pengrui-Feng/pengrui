...
path2 = '/content/drive/My Drive//PENGRUI/data_process_test/'
hour=3600*3 #match ctd and gps in 3 hours
#convert time format
...
for ctd_ptt,argos_date in tqdm(zip(dict_ctd['PTT'],dict_ctd['argos_date'])):
    ...
    for gps_ptt,gps_date in zip(dict_gps['PTT'],dict_gps['gps_date']):
        if(ctd_ptt == gps_ptt and  abs((datetime.strptime(argos_date, "%Y-%m-%d %H:%M:%S") - datetime.strptime(gps_date, "%Y-%m-%d %H:%M:%S")).total_seconds()) < hour):
...
print("\nwriting file: ")
with open('/content/drive/My Drive/PENGRUI/merge_split/'+db+'_merge_split.csv','w') as csvfile:
...
###convert to nosplit
df=pd.read_csv('/content/drive/My Drive/PENGRUI/merge_split/'+db+'_merge_split.csv')
